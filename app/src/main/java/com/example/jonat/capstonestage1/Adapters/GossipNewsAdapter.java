 ...
 */public class GossipNewsAdapter extends RecyclerView.Adapter<GossipNewsAdapter.CustomViewHolder> {
    ...
    private final NewsFeed newfeeds = new NewsFeed();

    public void setData(ArrayList<NewsFeed> feedList) {
            for (NewsFeed items : feedList){
                add(items);
            }
        }

    public void add(NewsFeed newsFeed){
        synchronized (newfeeds){
            feedItemList.add(newsFeed);
        }
    ...
    public GossipNewsAdapter(Context context, List<NewsFeed> feedItemList, Callbacks callbacks) {
